...
and third central moments of $B$ and $Y$.

\subsection{The Central Moments of the Binomial}
...
\subsection{The Central Moments of the Skew Normal}
...
To derive our approximation, we set the central moments of our two
distributions equal to each other and, taking $n$ and $p$ as constants, solve
for $\mu$, $\sigma$ and $\lambda$:
...
Finally, one last consideration that bears mentioning is that the skew-normal
approximation will sometimes give us parameters that violate our intuition. In
particular, for small $p$ and $n$, equation \eqref{eq:mu-solved} can yield
negative values of $\mu$, which is impossible for the binomial. This can be
explained by the fact that as $n$ and $p$ grow more extreme, so does the
inaccuracy of our approximation. Rather than impose a hard restriction on these
cases, we would simply advise that the reader be aware of the limits of the
skew-normal and, if more accuracy is necessary, seek another method of
approximation.

\section{Demonstrating Improved Accuracy}
\label{sec:demonstrating-improved-accuracy}
