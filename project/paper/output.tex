...
approximation will sometimes provide us with parameters that violate our
intuition. In particular, for small $p$ and $n$, equation \eqref{eq:mu-solved}
can yield negative values of $\mu$, which is impossible for the binomial. This
can be explained by the fact that as $n$ and $p$ grow more extreme, so does the
