...
\usepackage{jbib, jgraphics, jmath, jtable}
...
\begin{defn}
  ...
  \begin{equation} \label{eq:sn-pdf}
    f_Y(x|\mu, \sigma, \lambda) = \frac2\sigma \cdot \phi \left( \frac{x-\mu}{\sigma} \right) \cdot \Phi \left( \frac{\lambda(x-\mu)}{\sigma} \right), \quad x \in \R,
  \end{equation}
...
developed by \citet{azzalini-1985}. It belongs to a class of skewed
distributions conforming to the following guidelines:
...
In equation \eqref{eq:azzalini-skewed-distribution}, $f_0$ acts as a weighting
function referred to as a kernel. When $f_0 = \phi$, the distribution is said
to be generated by the normal kernel. $G$ does not necessarily have to be taken
from the same distribution as $f_0$; for example, the normal kernel has been
paired with cdf's from various other distributions, such as Student's t,
Cauchy, Laplace, and uniform. In the case of our skew-normal, however, both
$f_0$ and $G$ come from the normal distribution.

The skew-normal's first three moments about the origin (raw moments), given by
\citet{pewsey}, are
...
\lambda^2}}$. Using these raw moments, we can derive $Var(Y)$:

\begin{align}
  Var(Y) &= E(Y^2) - [E(Y)]^2 \nonumber \\
  &= \mu^2 + 2b \delta \mu \sigma + \sigma^2 - (\mu + b \delta \sigma)^2 \nonumber \\
  &= \cancel{\mu^2} + \cancel{2b \delta \mu \sigma} + \sigma^2 - \cancel{\mu^2} - \cancel{2b \delta \mu \sigma} - b^2 \delta^2 \sigma^2 \nonumber \\
  &= \sigma^2 - b^2 \delta^2 \sigma^2 \nonumber \\
  &= \sigma^2 (1 - b^2 \delta^2). \label{eq:sn-variance}
\end{align}
...
\begin{proof}
  ...
  \phi(t)\;dt$, \thinspace is not even, approaching 0 as $x \to -\infty$ and
  approaching 1 as $x \to \infty$. Thus,
...
\begin{proof}
  Lemma \ref{lem:azzalini-lemma-1} from section \ref{subsec:foundations}
  provides a very useful result for the above defined $f_0$ and $f$, which we
  restate here as a corollary:
  ...
  \begin{cor}[Perturbation Invariance]
    ...
    notation $\overset{d}{=}$ denotes equality in distribution. (Page 161,
    \citealp{azzalini})
  \end{cor}

  Let $X \sim N(0,1)$. Take $f_0 = \phi$, and $G = \Phi$. Then, $f_Z(z) = 2
  \cdot \phi(z) \cdot \Phi(\lambda z)$ conforms to Equation
  \eqref{eq:azzalini-skewed-distribution}, and we can conclude that $X$ and $Z$
  are equal in distribution.

  We will now show that $X^2 \sim \chi^2_1$ by deriving its moment generating
  function (mgf):\footnote{If $X$ is a random variable, then the expected value
  $M_X(t) = E(e^{tX})$ is called the moment generating function (mgf) of X if
  this expected value exists for all values of $t$ in some interval of the form
  $-h < t < h$ for some $h > 0$. Definition 2.5.1, \citet{textbook}.}
  ...
  $X$, we can also conclude that $Z^2 \sim \chi^2_1$. \end{proof}
...
\begin{proof}
  ...
  On $A_1$: $y = -x \Ra x = -y$, and the absolute value of the Jacobian $|\J| =
  \left| \frac{dx}{dy} \right| = |-1| = 1$, yielding
  ...
  Similarly, on $A_2$: $y = x \Ra x = y$, and the absolute value of the
  Jacobian $|\J| = \left| \frac{dx}{dy} \right| = |1| = 1$, yielding
  ...
  On $A_3$, we have $x = 0 \Ra y = 0$, and the absolute value of the Jacobian
  $|\J| = \left| \frac{dx}{dy} \right| = |0| = 0$, yielding $f_Y(y) = f_X(x)
  \cdot |\J| = f_X(x) \cdot 0 = 0$.
